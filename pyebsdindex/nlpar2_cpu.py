...
class NLPAR(nlpar_cpu.NLPAR):
  def __init__(self, filename=None,  lam=0.7, searchradius=3,dthresh=-3.0, diff_offset=0.0,
  ...
  def opt_lambda_cpu(self, target_weights=(0.5, 0.34, 0.25), dthresh=-3.0, autoupdate=True,
    ...
    def loptfunc(lam,d2,tw,dthresh):
      temp = np.select([d2 < dthresh], [0], default = d2)
      dw = np.exp(-(temp) / lam ** 2)
      w = np.sum(dw, axis=2) + 1e-12
      w2 = np.max(dw, axis=2)/w

      metric = np.median(np.abs(tw - w2))
      ...
      for i in numba.prange(colstartcount[0], colstartcount[0] + colstartcount[1]):
        ...
        for j_nn in range(nn_r_start, nn_r_end):
          for i_nn in range(nn_c_start, nn_c_end):
            ...
            if not ((i == i_nn) and (j == j_nn)): # the pattern of interest -- d2 will be zero.
              ...
              nout[j, i, count] = n2
              if d2 >= 1.e-3:  # sometimes EDAX collects the same pattern twice -- d2 will be 0.0
                s0 = d2 / np.float32(n2 * 2.0)
                if s0 < mind:
                  mind = s0
                dout[j, i, count] = d2  # want to save this for labmda optimization
            else:
                nout[j, i, count] = indices.size
            count += 1
        ...
        mediandtemp = np.median(np.sort(dtemp[dtemp > 1.0e-3])[0:5]) # make sure to get the lowest values.
        #mediandtemp = np.median(dtemp[dtemp > 1.0e-3])
        madtemp = np.median(np.absolute(dtemp[dtemp > 1.0e-3] - mediandtemp))
